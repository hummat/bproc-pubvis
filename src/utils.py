...
def init_renderer(resolution: int | Tuple[int, int], transparent: bool, look: Look | str, exposure: float,
                  engine: Engine | str, noise_threshold: float, samples: int):
...
def setup_obj(obj_path: str | Path | Tuple[Path, Path],
              center: bool = True,
              scale: bool = True,
              pcd: bool | int = False,
              ...
              point_color: Optional[Tuple[float, float, float] | Color | str] = None) -> bproc.types.MeshObject:
    ...
    """
    data = load_data(obj_path=obj_path, center=center, scale=scale, pcd=pcd, keep_mesh=keep_mesh)
    if isinstance(data, tuple):
        color = [color, 'plasma_r' if point_color is None else point_color]
    else:
        data = [data]
        color = [color]
...
def load_data(obj_path: str | Path | Tuple[Path, Path] | Primitive,
              center: bool = True,
...
def add_ambient_occlusion(obj: Optional[bproc.types.MeshObject] = None, distance: float = 0.2, strength: float = 0.5):
    ...
    if obj is None:
        ...
        try:
            ...
            Utility.insert_node_instead_existing_link(links, render_layers_node.outputs['Image'],
                                                      mix_rgb_node.inputs['Image'], mix_rgb_node.outputs['Image'],
        ...
        try:
            ...
            material.insert_node_instead_existing_link(attribute_node.outputs['Color'], ao_node.inputs['Color'],
...
def render_depth(obj: bproc.types.MeshObject,
    ...
    if pcd:
        ...
        set_color(obj=obj, color=color or 'plasma_r', camera_location=cam_location, instancer=point_shape is not None)
        init_pointcloud(obj=obj, point_size=point_size, point_shape=point_shape)
        bproc.camera.set_resolution(*resolution)
        if keep_mesh:
            obj.set_parent(mesh)
...
def create_mp4_with_ffmpeg(image_folder: Path, output_path: Path, fps: int = 20):
    ...
    ffmpeg_command = [
        'ffmpeg', '-framerate',
        str(fps), '-i',
        str(image_folder / 'image_%04d.png'), '-c:v', 'libx264', '-pix_fmt', 'yuv420p',
        str(output_path)
    ]
...
def make_animation(obj: bproc.types.MeshObject,
    ...
    if animation in [Animation.TURN, Animation.SWIVEL]:
        ...
        for frame, angle in enumerate(np.linspace(0, 360 - 360 // frames, frames)):
            rotate_obj(obj=pivot, rotate=(0, 0, -angle if animation is Animation.TURN else angle), frame=frame)
    ...
    if not debug:
        ...
        if save.suffix == '.gif':
            images[0].save(save, save_all=True, append_images=images[1:], format='GIF', duration=1000 // fps, loop=0)
