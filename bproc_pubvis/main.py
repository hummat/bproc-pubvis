...
from bproc_pubvis.constants import Animation, Color, Engine, Light, Look, Shading, Shadow, Shape, Strength
...
def run(cfg: Config):
    ...
    animate = (
        Animation.TURN
        if isinstance(cfg.animate, bool) and cfg.animate
        else Animation(cfg.animate)
        if cfg.animate
        else None
    )
    ...
    base_light = cfg.light if cfg.light is not None else (Light.BRIGHT if is_mesh else Light.VERY_BRIGHT)
